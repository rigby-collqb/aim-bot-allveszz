--[[
    ALLVESZ PRISON LIFE V9 (DEFINITIVE EDITION)
    - Jogo: Prison Life (Place ID: 155615604)
    - Combat: Aimbot, Wall Check, Team Check, Kill Check, Distância Mínima
    - Visual: Name ESP, 2D Box, Skeleton ESP (Novo), Tracers, Health Bar
    - Misc: Anti-Lag, Auto-Base (Criminal), Anti-Arrest (Taser)
    - Dev: 17gemadin
]]
...
local Settings = {
    ...
    Aimbot = false,
    WallCheck = true,
    TeamCheck = true, -- Padrão true no Prison Life
    AliveCheck = true,
    TargetPart = "Head",
    MinDistance = 15,
    ...
    -- Misc Prison Life
    AutoBase = false,     -- NOVO: Resetar ao virar criminoso
    AntiArrest = false,   -- NOVO: Resetar ao ser tased
...
local VisualsCache = {} -- Cache de desenhos
local HasResetForBase = false -- Controle do Auto-Base
    ...
    for _, cache in pairs(VisualsCache) do
        for _, drawing in pairs(cache) do
            ...
            elseif type(drawing) == "table" then
                -- Limpar tabelas aninhadas (como o esqueleto)
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer then
                    ...
                    if OnScreen then
                        ...
                        if Settings.ESP_Skeleton then
                            ...
                            if Head and Torso and LA and RA and LL and RL then
                                ...
                                local H_P, H_Vis = WorldToV2(Head.Position)
                                local T_P, _ = WorldToV2(Torso.Position)
                                local LA_P, _ = WorldToV2(LA.Position)
                                local RA_P, _ = WorldToV2(RA.Position)
                                local LL_P, _ = WorldToV2(LL.Position)
                                local RL_P, _ = WorldToV2(RL.Position)
            ...
            if not Show then
                ...
                D.HPBarOutline.Visible = false
                D.HPBarFill.Visible = false
                D.Skeleton.HeadCircle.Visible = false
                D.Skeleton.Spine.Visible = false
                D.Skeleton.LArm.Visible = false
                D.Skeleton.RArm.Visible = false
                D.Skeleton.LLeg.Visible = false
                D.Skeleton.RLeg.Visible = false
            end
        end
    end
...
local TabMisc = CreateTab("MISC (PL)")
...
AddToggle(TabVisual, "Skeleton (Esqueleto)", Settings.ESP_Skeleton, function(v) Settings.ESP_Skeleton = v end) -- NOVO
AddColorPicker(TabVisual, "Cor do Esqueleto", "SkeletonColorIdx", nil) -- NOVO
...
AddToggle(TabMisc, "Auto-Base (Criminals)", Settings.AutoBase, function(v) Settings.AutoBase = v end)
AddLabel(TabMisc, " * Reseta 1x ao virar Criminal para pegar armas.")

AddToggle(TabMisc, "Anti-Arrest (Taser)", Settings.AntiArrest, function(v) Settings.AntiArrest = v end)
AddLabel(TabMisc, " * Reseta se você for atingido por Taser.")
...
RunService.RenderStepped:Connect(function()
    ...
    -- PRISON LIFE MISC LOGIC
    local Char = LocalPlayer.Character
    local Hum = Char and Char:FindFirstChild("Humanoid")
    
    if Char and Hum and Hum.Health > 0 then
        
        -- AUTO-BASE (Reset once when criminal)
        if Settings.AutoBase then
            if LocalPlayer.Team.Name == "Criminals" then
                if not HasResetForBase then
                    HasResetForBase = true
                    Char:BreakJoints() -- Reset
                end
            else
                HasResetForBase = false -- Resetar a flag se mudar de time (ex: preso de novo)
            end
        end
        
        -- ANTI-ARREST (Detect Taser / PlatformStand)
        if Settings.AntiArrest then
            -- No Prison Life, ser tasado deixa o PlatformStand true
            if Hum.PlatformStand == true then
                Char:BreakJoints() -- Reset instantâneo para evitar algemas
            end
        end
    end
