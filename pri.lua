--[[
    ALLVESZ PRISON LIFE EDITION (V11 - UI FIXED)
    - Correção: Abas da UI agora funcionam corretamente.
...
local VisualsCache = {} 
local HasResetForCriminal = false

-- Limpeza de UI antiga
if CoreGui:FindFirstChild("AllveszPL_UI_V11") then CoreGui.AllveszPL_UI_V11:Destroy() end
...
local function UpdateSkeleton(Cache, Char, Color)
    ...
    if not (Parts.Head and Parts.Torso and Parts.LArm and Parts.RArm and Parts.LLeg and Parts.RLeg) then
        Cache.HeadCircle.Visible = false; Cache.Spine.Visible = false; Cache.LArm.Visible = false; Cache.RArm.Visible = false; Cache.LLeg.Visible = false; Cache.RLeg.Visible = false
        return
    end
    ...
    local LAPos, _ = GetScreenPos(Parts.LArm)
    local RAPos, _ = GetScreenPos(Parts.RArm)
    local LLPos, _ = GetScreenPos(Parts.LLeg)
    local RLPos, _ = GetScreenPos(Parts.RLeg)
                ...
                if not Settings.TeamCheck or v.Team ~= LocalPlayer.Team then
                    ...
                    if OnScreen then
                        ...
                        if Settings.ESP_Box then
                            ...
                            D.Box.Visible = true; D.Box.Size = Vector2.new(SizeX, SizeY); D.Box.Position = TopLeft; D.Box.Color = ColorList[Settings.BoxColorIdx].Color
                        else
                            D.Box.Visible = false
                        end
                        -- SKELETON
                        if Settings.ESP_Skeleton then
                            ...
                            D.HeadCircle.Visible = false; D.Spine.Visible = false; D.LArm.Visible = false; D.RArm.Visible = false; D.LLeg.Visible = false; D.RLeg.Visible = false
                        end
                    end
                end
            end
            if not Show then for _, obj in pairs(D) do obj.Visible = false end end
        end
    end
...
-- ==================================================================
-- INTERFACE (UI) REFEITA
-- ==================================================================
local ScreenGui = Instance.new("ScreenGui", CoreGui)
ScreenGui.Name = "AllveszPL_UI_V11"
...
TabPadding.Size = UDim2.new(1,0,60,0) -- Espaço para o titulo
...
PagesContainer.Position = UDim2.new(0, 180, 0, 10)
...
local function CreateTab(Name, Order)
    ...
    Page.ScrollBarImageColor3 = Color3.fromRGB(160, 30, 255)
    Page.AutomaticCanvasSize = Enum.AutomaticSize.Y -- ISSO CORRIGE O PROBLEMA DE NÃO APARECER
    ...
    TabBtn.MouseButton1Click:Connect(function()
        ...
        -- Ativar a atual
        Page.Visible = true
        TweenService:Create(TabBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(160, 30, 255), TextColor3 = Color3.fromRGB(255, 255, 255)}):Play()
    end)
    
    return Page, TabBtn -- Retorna a página e o botão
end

-- Criando as Abas
local TabCombat, BtnCombat = CreateTab("COMBATE", 1)
local TabVisual, BtnVisual = CreateTab("VISUAL", 2)
local TabMisc, BtnMisc = CreateTab("PL MISC", 3)
local TabCredits, BtnCredits = CreateTab("CRÉDITOS", 4)
...
local function AddToggle(Parent, Text, Default, Callback)
    ...
    Frame.MouseButton1Click:Connect(function()
        State = not State
        TweenService:Create(Checkbox, TweenInfo.new(0.2), {BackgroundColor3 = State and Color3.fromRGB(160, 30, 255) or Color3.fromRGB(40, 40, 40)}):Play()
        Callback(State)
    end)
end
...
AddToggle(TabVisual, "Esqueleto R6", Settings.ESP_Skeleton, function(v) Settings.ESP_Skeleton = v end)
AddColorPicker(TabVisual, "Cor Esqueleto", "SkeletonColorIdx", nil)
...
AddLabel(TabMisc, " * Reseta 1x ao virar Criminal.")

AddToggle(TabMisc, "Anti-Arrest (Taze)", Settings.AntiArrest, function(v) Settings.AntiArrest = v end)
AddLabel(TabMisc, " * Reseta se for paralisado.")

local BtnLag = Instance.new("TextButton", TabMisc)
BtnLag.Size = UDim2.new(0.98,0,0,40)
BtnLag.BackgroundColor3 = Color3.fromRGB(180,0,0)
BtnLag.Text = "REMOVER LAG"
BtnLag.TextColor3 = Color3.new(1,1,1)
BtnLag.Font = Enum.Font.GothamBold
BtnLag.TextSize = 14
...
CreditFrame.Size = UDim2.new(0.98,0,0,100)
CreditFrame.BackgroundColor3 = Color3.fromRGB(20,20,20)
Instance.new("UICorner", CreditFrame)
local DevTxt = Instance.new("TextLabel", CreditFrame)
DevTxt.Size = UDim2.new(1,0,1,0)
DevTxt.BackgroundTransparency = 1
DevTxt.Text = "Allvesz PL V11\nFeito por 17gemadin"
DevTxt.TextColor3 = Color3.new(1,1,1)
DevTxt.Font = Enum.Font.GothamBold
DevTxt.TextSize = 18

-- INICIALIZAÇÃO UI
-- Forçar a primeira aba a abrir
TabCombat.Visible = true
BtnCombat.BackgroundColor3 = Color3.fromRGB(160, 30, 255)
...
LocalPlayer:GetPropertyChangedSignal("Team"):Connect(function()
    if Settings.AutoBase and LocalPlayer.Team and LocalPlayer.Team.Name == "Criminals" then
...
RunService.RenderStepped:Connect(function()
    ...
    if Settings.AntiArrest and LocalPlayer.Character then
        ...
        if Hum then
            if Hum.PlatformStand or Hum:GetState() == Enum.HumanoidStateType.PlatformStanding or LocalPlayer.Character:FindFirstChild("Tased") then
