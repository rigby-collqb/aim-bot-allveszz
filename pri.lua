        ...
        if Settings.AutoBase then
            if LocalPlayer.Team.Name == "Criminals" then
                if not HasResetForBase then
                    ...
                    Hum.Health = 0 -- Reset garantido
                end
            else
                HasResetForBase = false
            end
        end
        
        -- ANTI-ARREST (ATUALIZADO V2)
        if Settings.AntiArrest then
            -- Verifica se está em PlatformStand (Tase padrão)
            if Hum.PlatformStand then
                Hum.Health = 0
            end

            -- Verifica se está "Sentado" no chão (Tase que derruba)
            -- A lógica 'not Hum.SeatPart' garante que você não morra se entrar num carro
