--[[
    ALLVESZ UNIVERSAL SCRIPT V9 PRO (DEFINITIVE EDITION)
    - Ícone: Letra "A" Estilizada
    - Combat: Aimbot, Wall Check, Team Check, Kill Check, Distância Mínima (Novo)
    - Visual 2.0: Name ESP, 2D Box, Tracers, Health Bar Dinâmica
...
local Settings = {
    ...
    AliveCheck = true,
    TargetPart = "Head",
    MinDistance = 15,     -- NOVO: Distância mínima para travar mira
    
    -- Visuals (ESP)
    ESP_Name = false,     -- Nome (Billboard)
    ESP_Box = false,      -- Box 2D (Drawing)
    ESP_Tracer = false,   -- Linhas (Drawing)
    ESP_Health = false,   -- Barra de Vida (Drawing)
    
    ESPColorIdx = 1,      -- Cor do Texto
    BoxColorIdx = 1,      -- Cor da Box
    TracerColorIdx = 5,   -- Cor do Tracer (Padrão Branco)
...
local VisualsCache = {} -- Armazena objetos de desenho (Box, Tracer, etc)

-- Limpeza de UI Antiga e Desenhos
if CoreGui:FindFirstChild("AllveszUI_V9") then CoreGui.AllveszUI_V9:Destroy() end
if CoreGui:FindFirstChild("AllveszESP") then CoreGui.AllveszESP:Destroy() end

-- Função para limpar desenhos antigos ao reiniciar
...
-- Busca de Alvo (Com filtro de Distância Mínima)
local function GetTarget()
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("Head") and v.Character:FindFirstChild("HumanoidRootPart") then
            ...
            -- NOVO: Check de Distância Mínima de Segurança
            local MyRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if MyRoot then
                local Dist3D = (MyRoot.Position - Root.Position).Magnitude
                if Dist3D < Settings.MinDistance then continue end -- Ignora se estiver muito perto
            end
...
-- Gerenciador de Desenhos (ESP Avançado)
...
local function UpdateVisuals()
            ...
            if Char and Hum and Root and Hum.Health > 0 then
                ...
                if not Settings.TeamCheck or v.Team ~= LocalPlayer.Team then
                    ...
                    if OnScreen then
                        ...
                        local SizeY = 2000 / BoxHeight -- Ajuste de escala
                        ...
                        if Settings.ESP_Tracer then
                            ...
                            D.Tracer.From = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y) -- Fundo da tela
                        ...
                        if Settings.ESP_Health then
                            ...
                            D.HPBarFill.Color = Color3.fromHSV(HealthPct * 0.3, 1, 1) -- Magia da cor (0.3 = Verde, 0 = Vermelho)
                        else
    ...
    -- Limpar cache de players que saíram
...
MainFrame.Size = UDim2.new(0, 600, 0, 420) -- Aumentei um pouco a altura
...
AddInput(TabCombat, "Distância Mínima (Segurança)", Settings.MinDistance, function(v) Settings.MinDistance = v end) -- NOVO
AddLabel(TabCombat, " * Inimigos mais perto que isso serão ignorados pelo aimbot")
...
-- 2. VISUAL (Atualizado V9)
AddLabel(TabVisual, "--- ESP NOME ---")
AddToggle(TabVisual, "Ativar Nomes (Wallhack)", Settings.ESP_Name, function(v) Settings.ESP_Name = v end)
AddColorPicker(TabVisual, "Cor do Nome", "ESPColorIdx", nil)

AddLabel(TabVisual, "--- VISUALS 2.0 (Desenhos) ---")
AddToggle(TabVisual, "Box 2D (Caixas)", Settings.ESP_Box, function(v) Settings.ESP_Box = v end) -- NOVO
AddColorPicker(TabVisual, "Cor da Box", "BoxColorIdx", nil) -- NOVO

AddToggle(TabVisual, "Tracers (Linhas)", Settings.ESP_Tracer, function(v) Settings.ESP_Tracer = v end) -- NOVO
AddColorPicker(TabVisual, "Cor da Linha", "TracerColorIdx", nil) -- NOVO

AddToggle(TabVisual, "Barra de Vida (Health Bar)", Settings.ESP_Health, function(v) Settings.ESP_Health = v end) -- NOVO
...
RunService.RenderStepped:Connect(function()
    ...
    -- VISUALS 2.0 (DRAWINGS UPDATE)
    UpdateVisuals()

    -- ESP NAME LOGIC (Billboard - Legacy V8 Style but Toggleable)
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "Allvesz V9 PRO",
    Text = "Script Atualizado com Sucesso!",
    Duration = 5
})
