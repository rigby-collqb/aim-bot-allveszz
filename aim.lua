...
local function UpdateESP()
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("Head") and v.Character:FindFirstChild("Humanoid") then
            ...
            -- Sincronizar Adornee e Cor
            if BB.Adornee ~= v.Character.Head then BB.Adornee = v.Character.Head end
            BB.Label.TextColor3 = ColorList[Settings.ESPColorIndex].Color
        else
            -- Jogador saiu ou está sem personagem
...
RunService.RenderStepped:Connect(function()
    ...
    -- SOLTAR A MIRA
    local TargetHum = LockedTarget.Parent:FindFirstChild("Humanoid")
    if not TargetHum or TargetHum.Health <= 0 then
        LockedTarget = nil -- Alvo morreu, solta a mira
    end
end

    -- Atualiza ESP a cada frame (Suave)
    UpdateESP()
end)
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "Allvesz V5",
    Text = "Script Carregado! GOD MODE Ativo.",
    Duration = 5
})
