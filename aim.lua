...
local function ActivateAntiLag()
    ...
    for _, v in pairs(workspace:GetDescendants()) do
        if v:IsDescendantOf(LocalPlayer.Character or Instance.new("Folder")) then continue end
...
local function CreateToggle(text, defaultVal, callback)
    ...
    Btn.MouseButton1Click:Connect(function()
        ...
        Indicator.BackgroundColor3 = newState and Theme.Purple or Color3.fromRGB(50,50,50)
        callback(newState)
    end)
end
...
CreateFOVInput()
CreateToggle("DESENHAR FOV", Settings.ShowFOV, function(v) Settings.ShowFOV = v end)
...
CreateActionButton("DESTRUIR LAG (FPS BOOST)", ActivateAntiLag)
...
local function UpdateESP()
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("Head") then
            ...
            BB.Label.TextColor3 = ColorList[Settings.ESPColorIndex].Color
        else
            if ESPFolder:FindFirstChild(v.Name) then ESPFolder[v.Name]:Destroy() end
        end
    end
